<?php $this->load->view('admin/post/head')?>
<!-- head -->
<div class="line"></div>
<div id="main_post" class="wrapper">
	<div class="widget">
		...
		<thead class="filter"><tr><td colspan="12">
				<form method="get" action="<?php echo admin_url('post')?>" class="list_filter form">
					<table width="80%" cellspacing="0" cellpadding="0"><tbody>
						...
						<tr>
										...
										<?php if(count($row->sub_menu) >= 1):?>
    				      				<optgroup label="<?php echo $row->title?>">
    				      				    <?php foreach ($row->sub_menu as $sub):?>
    				               			<option value="<?php echo $sub->id?>" <?php echo ($this->input->get('catalog') == $sub->id) ? 'selected' : ''?>> <?php echo $sub->title?> </option>
    						                <?php endforeach;?>
    				               		</optgroup>
    				               		<?php else:?>
    				               		  <option value="<?php echo $row->id?>" <?php echo ($this->input->get('catalog') == $row->id) ? 'selected' : ''?>><?php echo $row->title?></option>
    				               		<?php endif;?>
    				               		<?php endforeach;?>
								</select>
							</td>
							...
							<input type="reset" onclick="window.location.href = '<?php echo admin_url('post')?>'; " value="Reset" class="basic">
							</td>
				...
				<tr>
					<td colspan="12">
						 <div class="list_action itemActions">
								<a url="<?php echo admin_url('post/delete_all')?>" class="button blueB" id="submit" href="#submit">
			...
			<tbody class="list_item">
			     ...
			     <tr class="row_<?php echo $row->id?>">
					...
					<div class="image_thumb">
						<img height="50" src="<?php echo !empty($row->image_link) ? $row->image_link : ''?>">
						<div class="clear"></div>
					</div>
					...
					<td class="option textC">
						<a title="Xem chi tiết sản phẩm" class="tipS" target="_blank" href="<?php echo base_url($row->slug_catalog.'/'.$row->slug.'.html')?>">
						 ...
						 <a class="tipS" title="Chỉnh sửa" href="<?php echo admin_url('post/edit/'.$row->id)?>">
							<img src="<?php echo public_url('admin/images')?>/icons/color/edit.png">
						</a>
						
						<a class="tipS verify_action" title="Xóa" href="<?php echo admin_url('post/del/'.$row->id)?>">
